...
(def ^:private mutable-update-fields
  #{:monitored :quality-profile-id :minimum-availability :movie-metadata-id :last-search-time})
  ...
  (let [errors (model/validate movie)]
    (if (seq errors)
      ...
      (let [canonical      (model/normalize movie (clock env))
        ...
        (cond duplicate-meta     (let [response (duplicate-response :tmdb-id "duplicate-metadata" duplicate-meta)]
                                   (t/log! {:level   :info
          ...
          (let [{:keys [errors movie]} (prepare-update movie-id existing patch (clock env))]
            (if (seq errors)
              (do (t/log! {:level :warn :reason :movies/update-invalid :details {:movie-id movie-id :errors errors}}
                  ...
                  {:status :invalid :errors errors})
              (let [metadata-id (:movie-metadata-id movie)
                    conflict-id (when metadata-id (pstate/movie-id-by-metadata-id env metadata-id))]
                (cond
                  (and conflict-id (not= conflict-id movie-id)) (let [conflict (or (pstate/movie-by-id env conflict-id)
                                                                                   {:id conflict-id})
                                                                      response (duplicate-response :tmdb-id
                                                                                                   "duplicate-metadata"
                                                                                                   conflict)]
                                                                  (t/log! {:level   :info
