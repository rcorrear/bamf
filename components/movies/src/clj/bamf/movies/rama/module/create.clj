(ns bamf.movies.rama.module.create
  ...
  (:require [bamf.movies.rama.module.helpers :as helpers]
            [bamf.movies.rama.module.state :refer
             [$$metadata-by-movie-id $$movies $$movies-id-by-tmdb-id $$movies-ids-by-monitor $$movies-ids-by-monitored
              $$movies-ids-by-tag $$movies-ids-by-target-system]]
...
(deframaop movie-create
  [{:keys [*certification *clean-original-title *clean-title *collection *digital-release *genres *images *imdb-id
           *in-cinemas *last-info-sync *minimum-availability *monitor *monitored *movie-file-id *original-language
           *original-title *overview *path *physical-release *popularity *quality-profile-id *ratings *recommendations
           *root-folder-path *runtime *secondary-year *sort-title *status *studio *tags *target-system *title
           *title-slug *tmdb-id *website *year *you-tube-trailer-id]}]
  (<<with-substitutions [$$movies                      (this-module-pobject-task-global "$$movies")
    ...
    (<<if (nil? *existing-movie-id)
      ...
      (identity (->> {:certification        *certification
                ...
                *metadata)
      (<<if (seq *metadata)
        (helpers/log-map-value-types *metadata)
