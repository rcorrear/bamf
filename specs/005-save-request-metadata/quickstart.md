...
5) Tests  
- Add clojure.test/kaocha coverage for: valid metadata save/echo, metadata-less saves, PUT-only metadata updates, `null` removal behavior, status/minimumAvailability exact-match validation, rejection of invalid shape/status, ignoring unknown keys, and full payload delivery to Rama via `foreign-append!` (unit + module integration).  
