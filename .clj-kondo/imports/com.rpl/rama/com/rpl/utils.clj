...
(defn prnm
  ...
  [& x]
  (binding [*print-meta* true] (apply prn x)))
...
(defn sppy
  ...
  a symbol."
  ([x] (sppy "SPY:" x))
  ([p x] (print p " ") (pprint/pprint x) x))

(defn spy ([x] (spy "SPY:" x)) ([p x] (prn p x) x))
  ...
  (if token
    (let [symbol (api/sexpr token)]
      (if (qualified-symbol? symbol)
        (let [{:keys [name ns]} (api/resolve {:name symbol})] (and (= ns 'com.rpl.rama) (contains? symbol-set name)))
        (contains? symbol-set symbol)))
    false))

(defn rama= [a b] (rama-contains? #{a} b))
