...
(defn flatten-all
  ...
  [x]
  (filter (complement coll?) (rest (tree-seq coll? seq x))))
...
(defn emit-node?
  ...
  (when (api/keyword-node? node)
    (let [k (str (:k node))] (and (str/ends-with? k ">") (not (some #{\space \newline} k))))))

(defn pstate-or-nil
  "Returns the token if it represents a pstate. Otherwise, return `nil`"
  [token]
  (when (and (symbol? token) (str/starts-with? (str token) "$$")) token))

(defn ramavar-or-nil
  "Given a string, return the ramavar it represents.
  If it's not a ramavar, return `nil`"
  [token]
  (when (and (string? token) (contains? #{\* \! \% \$} (first token)) (not (some #{\space \newline} token)))
    (symbol token)))
...
(defn separate-new-and-old-bindings
  ...
  "
  [existing maybe-new]
  [(set/difference maybe-new existing) (set/intersection existing maybe-new)])
...
(defn find-all-ramavars
  ...
  (when expr
    (let [expr     (api/sexpr (if (node? expr) [expr] expr))
          metadata (meta expr)]
      (with-meta (into #{} (comp (map ramavar-or-nil) (remove nil?) (map #(with-meta % metadata))) (flatten-all expr))
                 metadata))))
...
(defn remove-instances
  ...
  [sexpr symbol-set]
  (walk/postwalk (fn [x]
                   (cond (list? x)      (concat (empty x) (remove #(contains? symbol-set %) x))
                         (map-entry? x) x
                         (coll? x)      (into (empty x) (remove #(contains? symbol-set %) x))
                         :else          x))
                 sexpr))
...
(defn- inject-ramavars-map
  ...
  (let [fval (:value (second (:children (first body))))]
    (if (rama-contains? '#{case> default>} fval)
      (first body)
      (wrap-do (concat (butlast body)
                       (if (api/list-node? (last (:children (last body))))
                         [(api/list-node (concat (butlast (:children (last body)))
                                                 [(inject-ramavars-map ramavars [(last (:children (last body)))])]))]
                         [(api/list-node (concat (:children (last body))
                                                 [(api/map-node (mapcat (fn [ramavar] [(api/token-node ramavar)
...
(defn eliminate-unchecked-branches
  ...
  (->> (map vector branches bodies)
       ...
       (remove (fn [[[branch]]] (rama-contains? '#{case> default>} (:value (first (:children branch))))))
...
(def illegal-forms
  '#{declare def defonce defn defn- definline defmacro quote var memfn let letfn set! and or when when-not when-let
     when-first when-some if if-not if-let if-some cond condp case loop recur doseq dotimes for while binding locking
     time with-in-str with-out-str with-precision with-local-vars with-open with-redefs with-redefs-fn do try catch
     finally throw as-> cond-> cond->> some-> some->> .. doto defprotocol extend-type extend-protocol defrecord deftype
     defmethod defmulti definterface proxy ns import vswap! monitor-enter monitor-exit})
...
(defn java-method?
  ...
  (let [form-str (str form)]
    ...
    (or (str/starts-with? form-str ".") (str/ends-with? form-str "."))))

(defn keyword-fn?
  [form]
  (let [form-str (str form)]
    (and (str/starts-with? form-str ":")
         ;; If it ends with ">" then it's an emit token ex. :> or :err>
...
(defmulti ^:private validate-form
  (fn [form]
    (let [value (or (:value form) (:k form) form)]
      (cond (keyword-fn? value)             :keyword-fn-form
            (= 'fn value)                   :lambda-fn-form
            (contains? illegal-forms value) :special-form
            (java-method? value)            :java-form
            :else                           form))))

(defmethod validate-form :default [_form] true)

(defmethod validate-form :keyword-fn-form [form] (err/maybe-illegal-keyword-fn *context* (meta form)))

(defmethod validate-form :lambda-fn-form [form] (err/maybe-illegal-lambda *context* (meta form)))

(defmethod validate-form :special-form [form] (err/maybe-illegal-special-form *context* (:value form) (meta form)))

(defmethod validate-form :java-form [form] (err/maybe-illegal-java-form *context* (meta form)))
    ...
    (loop [terms                all-terms
      ...
      (let [current-term (first terms)
        ...
        (cond
          ...
          (empty? terms) (let [;; Calculate final vars based on collected output-vars
          ...
          :else (throw (ex-info "Unexpected state in extract-emits" {:state state :term current-term})))))))
...
(defn- extract-binding-emits
  ...
  [terms curr-ramavars]
  (if (and (api/keyword-node? (first terms)) (= :> (:k (first terms))))
    [[] (rest terms)]
...
(defmulti ^:private split-form
  ...
  (fn [_f children _ramavars]
    (let [block-type (:value (first children))]
      (if (qualified-symbol? block-type) (:name (api/resolve {:name block-type})) block-type))))

(defmethod split-form :default [_f _children _ramavars] {:prefix nil :branches nil})
...
(defmethod split-form '<<sources
  ...
  (let [metadata                         (meta f)
        ...
        blocks                           (partition-by #(and (api/list-node? %)
                                                             (rama= 'source> (:value (first (:children %)))))
                                                       (if missing-topology? (rest children) (drop 2 children)))
...
(defmethod split-form '<<subsource
  ...
  (let [metadata                      (meta f)
        ...
        blocks                        (partition-by #(and (api/list-node? %)
                                                    ...
                                                    sources)
        branches                      (partition-all 2 blocks)]
    {:prefix   [(api/token-node 'case) (api/list-node (list (api/token-node 'type) data))]
     :branches (mapcat (fn [[[marker] body]]
                         (let [[_case> type-node] (:children marker)
                               marker-node        (api/list-node (list (api/token-node 'case>) type-node))]
...
(defmethod split-form '<<switch
  ...
  (let [m      (meta f)
        blocks (->> (drop 2 children)
                    (partition-by #(and (api/list-node? %) (rama= 'case> (:value (first (:children %))))))
                    (partition-all 2))]
    {:prefix   [(api/token-node 'cond)]
     :branches (mapcat (fn [[[marker] body]]
                         ...
                         [[(api/list-node (list (api/token-node 'case>)
                                                (api/list-node (list* (api/token-node '=)
                                                                      (second children)
...
(defmethod split-form '<<cond
  ...
  (let [m      (meta f)
        blocks (->> (rest children)
                    (partition-by #(and (api/list-node? %)
                                        (rama-contains? '#{case> default>} (:value (first (:children %))))))
                    (partition-all 2))]
    {:prefix   [(api/token-node 'cond)]
     :branches (mapcat (fn [[[marker] body]]
...
(defmethod split-form '<<if
  [f children _ramavars]
  (let [m                                         (meta f)
        [if-block else-marker else-block & extra] (partition-by #(and (api/list-node? %)
                                                                      (rama= 'else> (:value (first (:children %)))))
                                                                (drop 2 children))]
    (err/maybe-else-arity else-marker m)
    (err/maybe-multiple-elses extra m)
    {:prefix   [(api/token-node (if else-marker 'if 'when)) (second children)]
     :branches (if else-marker [[(wrap-<<do if-block)] [(wrap-<<do else-block)]] [if-block])}))
  ...
  (fn [node _following _ramavars]
    (if (emit-node? (first (:children node)))
      ...
      (let [value (:value (first (:children node)))]
        (if (qualified-symbol? value) (:name (api/resolve {:name value})) value)))))

(defmethod handle-form :default [_node _following _ramavars] nil)

(defmethod handle-form :emit
  [node following _ramavars]
  (if (= 1 (count (:children node)))
    [(with-meta (api/list-node (list (first (:children node)) (api/token-node nil))) (meta node)) following]
    nil))
...
(defmethod handle-form 'java-macro!
  ...
  (let [[_java-macro!-token expr] (:children node)
        found-vars                (into #{} (comp (map ramavar-or-nil) (remove nil?)) (api/sexpr expr))
        [new old]                 (separate-new-and-old-bindings ramavars found-vars)
        new-bindings              (api/vector-node (map api/token-node new))
        use-ramavars              (api/list-node (list (api/token-node 'pr) new-bindings))
        ...
        new-node                  (api/list-node
                                   (list* (api/token-node 'let)
                                          (api/vector-node [(api/token-node '_)
                                                            (api/list-node (list* (api/token-node 'pr)
                                                                                  (map api/token-node old)))
                                                            new-bindings (api/vector-node [])])
                                          (transform-body (concat [use-ramavars] [linted-expr] following) ramavars)))
        metadata                  (meta node)]
  ...
  (let [outvars (->> (api/sexpr node)
    ...
    (if (seq outvars)
      (let [[_ $$pstate template] (:children node)
            fake-node             (api/list-node (list (api/token-node 'pr) $$pstate template))]
        [(with-meta (api/list-node (list* (api/token-node 'let)
                                          (api/vector-node [(api/vector-node (map api/token-node outvars))
                                                            (api/vector-node [])])
...
(defmethod handle-form 'batch<-
  ...
  (let [[_ input & body] (:children node)
        ...
        new-node         (api/list-node (list (api/token-node 'fn)
                                              (api/vector-node [])
                                              (api/list-node (list* (api/token-node 'let)
                                                                    (api/vector-node (mapcat #(vector (api/token-node %)
                                                                                                      (api/string-node
                                                                                                       (str %)))
                                                                                      pstates))
...
(defmethod handle-form '<<query-topology
  ...
  (let [metadata                               (meta node)
        ...
        [input new-bindings]                   (extract-binding-emits (:children input-output) #{})
        ret-node                               (with-meta (api/vector-node new-bindings) metadata)
...
(defn handle-frags
  ...
  (let [metadata              (meta node)
        ...
        new-node              (api/list-node (list* (api/token-node 'letfn)
                                                    (api/vector-node [(api/list-node
                                                                       (list* name input (transform-body body)))])
...
(defmethod handle-form '<<ramaop [node following ramavars] (handle-frags node following ramavars))

(defmethod handle-form '<<ramafn [node following ramavars] (handle-frags node following ramavars))
...
(defn handle-loops
  ...
  (let [metadata                   (meta node)
        ...
        new-node                   (api/list-node (list* (api/token-node 'let)
                                                         (api/vector-node [(api/vector-node outputs)
                                                                           (api/list-node (list*
                                                                                           ;; NOTE: it would be more
                                                                                           ;; semantic to turn this
                                                                                           ;; into a `loop` form,
                                                                                           ;; but then we'd get
                                                                                           ;; "loop without recur"
                                                                                           ;; warnings.
...
(defmethod handle-form 'loop<- [node following ramavars] (handle-loops node following ramavars))

(defmethod handle-form 'async-loop<- [node following ramavars] (handle-loops node following ramavars))
...
(defmethod handle-form 'depot-partition-append!
  ...
  (let [depot-name (:value (second (:children node)))]
    ...
    (if (rama-contains? ramavars depot-name)
      ...
      (let [metadata (meta node)
            new-node (api/list-node (list* (api/token-node 'let)
                                           (api/vector-node [(api/token-node depot-name) (api/token-node nil)])
                                           (transform-body (concat [node] following) (conj ramavars depot-name))))]
        [(with-meta new-node metadata) nil]))))
...
(defmethod handle-form '<<do
  ...
  (let [children     (rest (:children node))
        ...
        new-node     (api/list-node (list* (api/token-node 'do) body))]
    [(with-meta new-node (assoc (meta node) ::ramavars ramavars)) nil]))

(defmethod handle-form '+group-by
  [node following ramavars]
  (let [children (:children node)
        new-node (api/list-node (list* (transform-body (concat children following) ramavars)))]
    [(with-meta new-node (meta node)) nil]))

(defmethod handle-form 'clj! [node following _ramavars] [node following])
...
(defn transform-form*
  ...
  ([f following ramavars]
   (when (api/list-node? f) (let [out (validate-form (first (:children f)))] out))
   (if-let [children (and (not (api/token-node? f)) (not (api/keyword-node? f)) (:children f))]
       ...
       (if (seq branches)
         (let [bodies   (mapv #(transform-body % ramavars) branches)
               ramavars (when (and (> (count branches) 1)
                                   (not (rama-contains? '#{<<sources} (:value (first children)))))
                          ...
                          (set/difference (reduce set/intersection (eliminate-unchecked-branches branches bodies))
                                          ramavars))]
             ...
             (let [follows (transform-body following ramavars)]
               [(with-meta (api/list-node
                            (list* (api/token-node 'let)
                                   (api/vector-node
                                    (concat
                                     ;; Stick the unified bindings up at the top of the `let`. This doesn't really
                                     ;; matter, but it's to handle the specific case where there's a `(default>
                                     ;; :unify false)` And the body of the branch ends up having the unified vars
                                     ;; injected into it. Having the unified vars declared first will ensure this
                                     ;; doesn't cause an unresolved symbol error
                                     (mapcat (fn [ramavar] [(api/token-node ramavar)
                                     ...
                                     [(api/token-node '_)
                                      (api/list-node (into prefix
                                                           ...
                                                           (map #(inject-ramavars-map ramavars %) bodies)))]))
                                   follows))
                           {::ramavars ramavars}) nil])
             [(with-meta (api/list-node (apply concat prefix bodies)) {::ramavars ramavars}) following]))
         (let [[expr out new-bindings rebinds new-vars found-anchors]
               (if (api/list-node? f) (extract-emits children ramavars) [f])]
           (if (or new-bindings rebinds (not-empty found-anchors))
             [(let [ramavars     (into ramavars new-vars)
                    ...
                    new-bindings (if rebinds
                                   (case (count new-bindings)
                                     0 []
                                     1 [(api/token-node (first new-bindings)) (api/list-node expr)]
                                     [(api/vector-node (mapv api/token-node new-bindings)) (api/list-node expr)])
                                   [(if (= 1 (count out)) (first out) (api/vector-node out)) (api/list-node expr)])
                    rebindings   (mapcat #(vector (api/token-node %) (api/list-node [(api/token-node 'identity) %]))
                                  rebinds)
                    anchor-binds (mapcat #(vector % (api/token-node nil)) found-anchors)
                    bind-expr    (with-meta (api/list-node (list* (api/token-node 'let)
                                                                  (api/vector-node
                                                                   (concat anchor-binds new-bindings rebindings))
                                                                  follows))
                                            {::ramavars ramavars})
                    ;; Produces
                    ;; (do
                    ...
                    ;;   (let [...vars ...(str vars)]
                    ;;     ...follows))
                    result       (with-meta (if rebinds
                                              (api/list-node (list (api/token-node 'do) (api/list-node expr) bind-expr))
                                              bind-expr)
                                            {::ramavars ramavars})]
                result) nil]
               ...
               (if handled-form
                 ...
                 [(let [;; HACK: same as above, we insert a call to trampoline
                        ...
                        children (if (and (api/list-node? f)
                                          (not (api/keyword-node? (first children)))
                                          (not (contains? '#{let letfn} (:value (first children)))))
                                   (cons (api/token-node 'trampoline) children)
                                   children)
                        body     (transform-body children ramavars)]
                    (vary-meta (cond (api/vector-node? f) (api/vector-node body)
                                     (api/map-node? f)    (api/map-node body)
                                     (api/set-node? f)    (api/set-node body)
                                     :else                (api/list-node body))
                               assoc
                               ::ramavars
                               (::ramavars (meta body)))) following]))))))
     [f following])))

(defn transform-form
  ([f following] (transform-form* f following #{}))
  ([f following ramavars] (let [[_node _following :as out] (transform-form* f following ramavars)] out)))
     ...
     (if (seq body)
       (let [[r following] (transform-form (first body) (rest body) ramavars)]
         (recur
          ...
          (vary-meta (conj result r) update ::ramavars set/union (::ramavars (meta result)) (::ramavars (meta r)))))
       result))))

(def module-declaration-forms
  '#{declare-depot declare-tick-depot declare-object declare-pstate mirror-depot mirror-pstate mirror-query})
...
(defn transform-module-form
  ...
  (if-let [children (and (not (api/token-node? form)) (not (api/keyword-node? form)) (:children form))]
    (cond
      ...
      (rama-contains? module-declaration-forms (:value (first children))) (let [[declare on name & definition] children
                                                                                pobject-name (:value name)
                                                                                pobjects (conj pobjects pobject-name)
                                                                                follows (transform-module-body following
                                                                                                               pobjects)
                                                                                pobjects (set/union pobjects
                                                                                                    (::pobjects
                                                                                                     (meta follows)))]
                                                                            (err/maybe-missing-pobject-name name
                                                                                                            declare
                                                                                                            (meta form))
                                                                            [(with-meta
                                                                               (api/list-node
                                                                                (list*
                                                                                 ...
                                                                                 (api/vector-node
                                                                                  [name
                                                                                   (api/list-node
                                                                                    (list* declare on definition))])
                                                                                 ;; We want to add this extra
                                                                                 ;; "call" to pr here to avoid
                                                                                 ;; getting warnings that the
                                                                                 ;; pstate is unused when it's
                                                                                 ;; referenced in a defgenerator or
                                                                                 ;; something like that
                                                                                 (api/list-node
                                                                                  (list (api/token-node 'pr) name))
                                                                                 follows))
                                                                               {::pobjects pobjects}) nil])
      ...
      (rama= '<<sources (:value (first children)))                        (let [[out _following]
                                                                                (binding [*context* :dataflow]
                                                                                  (transform-form form [] pobjects))]
                                                                            [out following])
      (rama= '<<query-topology (:value (first children)))                 (let [[out _following]
                                                                                (binding [*context* :dataflow]
                                                                                  (handle-form form [] pobjects))]
                                                                            [out following])
      :else                                                               [(let [body (transform-module-body children
                                                                                                             pobjects)]
                                                                             (vary-meta (cond (api/vector-node? form)
...
(defn transform-module-body
  ...
  (loop [body     body
         ...
         result   []]
    (if (seq body)
      (let [[r following] (transform-module-form (first body) (rest body) pobjects)]
        (recur
         following
         (into pobjects (::pobjects (meta r)))
         (vary-meta (conj result r) update ::pobjects set/union (::pobjects (meta result)) (::pobjects (meta r)))))
      result)))
...
(defn defbasicblocksegmacro-hook
  ...
  (let [[_ macro-name & other] (:children node)
        ...
        new-node               (with-meta (api/list-node (list* (api/token-node 'defn)
                                                                macro-name
                                                                (api/vector-node (filterv (complement :k)
                                                                                          (:children binding)))
                                                                body))
                                          (meta node))]
...
(defn defsegmacro-hook
  ...
  (let [m                     (meta node)
        ...
        input                 (->> (:children input)
                                   ...
                                   (api/vector-node))
        new-node              (api/list-node (list* (api/token-node 'defn) name input body))]
...
(defn deframaop-hook
  ...
  (let [m                         (meta node)
        ...
        new-node                  (api/list-node (list* (api/token-node 'defn)
                                                        name
                                                        input
                                                        (binding [*context* :dataflow] (transform-body children))))]
    (err/maybe-missing-def-name name m)
    (err/maybe-missing-input-vector input m)
    {:node (with-meta new-node m)}))
...
(defn top-level-block-hook
  ...
  (let [[_ & body] (:children node)
        new-node   (with-meta (api/list-node (list* (api/token-node 'do)
                                                    (binding [*context* :dataflow] (transform-body body))))
                              (meta node))]
    {:node (with-meta new-node (meta node))}))
...
(defn batch<--hook
  ...
  [{:keys [node]}]
  (let [metadata   (meta node)
        [new-node] (binding [*context* :dataflow] (transform-body [node]))]
...
(defn module-hook
  ...
  (let [children     (rest (:children node))
        ...
        [input body] (if (api/map-node? (first children))
                       (let [[_options input & body] children] [input body])
                       (let [[input & body] children] [input body]))]
    (err/maybe-missing-input-vector input (meta node))
    {:node (with-meta (api/list-node (list* (api/token-node 'fn) input (transform-module-body body #{})))
                      (meta node))}))
...
(defn defmodule-hook
  ...
  (let [m                 (meta node)
        ...
        [name input body] (if (api/map-node? (second children))
                            (let [[name _options input & body] children] [name input body])
                            (let [[name input & body] children] [name input body]))
        new-node          (with-meta (api/list-node
                                      (list* (api/token-node 'defn) name input (transform-module-body body #{})))
                                     (meta node))]
    (err/maybe-missing-def-name name m)
    (err/maybe-missing-input-vector input m)
    {:node (with-meta new-node m)}))
