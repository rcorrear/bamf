...
(defn defalias
  ...
  (let [[alias src-raw _attrs body] (rest (:children node))
    ...
    {:node (with-meta (hooks/list-node [(hooks/token-node 'def) (hooks/token-node sym)
                                        (if body
                                          (hooks/list-node
...
(defn defaliases
  ...
  (let [alias-nodes (rest (:children node))]
    {:node (hooks/list-node
            (into [(hooks/token-node 'do)]
                  (map (fn alias->defalias [alias-node]
                         (cond
                           (hooks/token-node? alias-node) (hooks/list-node [(hooks/token-node 'taoensso.encore/defalias)
                                                                            alias-node])
                           (hooks/map-node? alias-node)   (let [{:keys [src alias attrs body]} (hooks/sexpr alias-node)
                                                                ;; workaround as can't seem to (get) using a
                                                                ;; token-node and there's no update-keys (yet) in
                                                                ;; sci apparently
                                                                [& {:as node-as-map}]          (:children alias-node)
                                                                {:keys [attrs body]}           (zipmap
                                                                                                (map hooks/sexpr
                                                                                                     (keys node-as-map))
                                                                                                (vals node-as-map))]
                                                            (hooks/list-node [(hooks/token-node
                                                                               'taoensso.encore/defalias) (or alias src)
                                                                              (hooks/token-node src) attrs body])))))
                  alias-nodes))}))
...
(defn defn-cached
  ...
  (let [[sym _opts binding-vec & body] (rest (:children node))]
    {:node (hooks/list-node
            (list (hooks/token-node 'def) sym (hooks/list-node (list* (hooks/token-node 'fn) binding-vec body))))}))
...
(defn defonce
  ...
  (let [[sym & args]         (rest (:children node))
        ...
        rewritten            (hooks/list-node [(hooks/token-node 'clojure.core/defonce) sym+meta init-expr])]
    {:node rewritten}))
